...
with open(filePath + "/toolkit/relationStaticResult.txt", "r", encoding='utf8') as fr:
    for line in fr:
        ...
        try:
            ...
            relationCountDict[relationName] = 0
...
def sortDict(relationDict):
    ...
    relationDict = sorted(relationDict, key=lambda item: item[
                          'relationCount'], reverse=True)
...
def decisions_making(request):  # index页面需要一开始就加载的内容写在这里
    ...
    if request.POST:
        ...
        best_match += '<div class="col-md-9"><h1>' + \
            entity_list[0]['label_name'] + '</h1>'
        ...
        best_match += str(entity_list[0]['label_confd'] * 100) + \
            '%;" aria-valuemin="0" aria-valuemax="100">'
        best_match += str(entity_list[0]['label_confd'])[:7] + '</div></div>'
        best_match += '<h4><a href="detail.html?title=' + \
            entity_list[0]['label_name'] + '">[查看详细]</a></h4></div></div>'
        ...
        for i in range(1, len(entity_list)):
            ...
            other_match += '<img src="' + \
                entity_list[i]['image'] + \
                '" height="100%" width="100%"/></a></div>'
            other_match += '<div class="col-md-7"><h4>' + \
                entity_list[i]['label_name'] + '</h4>'
            ...
            other_match += str(entity_list[i]['label_confd'] * 100) + \
                '%;" aria-valuemin="0" aria-valuemax="100">'
            other_match += str(entity_list[i]['label_confd']
                               )[:7] + '</div></div></div></div>'
