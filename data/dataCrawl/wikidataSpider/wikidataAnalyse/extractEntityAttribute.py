...
# 加载实体列表
entitySet = set()
with open(filePath + "/4 wikidataProcessing/new_node.csv", 'r') as fr:
    newNodeCsv = csv.reader(fr)
    for item in newNodeCsv:
        entitySet.add(item[0].strip())
with open("../../../../entity process/KNN_predict/predict_labels.txt", 'r') as fr:
    for line in fr:
        predictLabel = line.split()[0]
        entitySet.add(predictLabel)

with open('../../../data process/hudong_pedia.csv', 'r') as fr:
            ...
            if(baseInfoKeyLength == baseInfoValueLength):
                for i in range(baseInfoValueLength):
                    ...
                    if(baseInfoValue in entitySet):
                        fw.write(entityName + "," + baseInfoKey +
                                 "," + baseInfoValue + '\n')
print('done.')
