...
class entityRelationSpider(scrapy.spiders.Spider):
    ...
    def parse(self, response):
        ...
        if(os.path.exists(os.path.join(filePath, "entity1_entity2.json"))):
            ...
            with open(os.path.join(filePath, "entity1_entity2.json"), 'r') as fr:
                for line in fr:
                    ...
                    alreadyGet.append(
                        entityIds['entity1'] + entityIds['relatedEntityId'])
        with open(filePath + "/3wikidataRelation/relationResult.json", "r") as fr:
        ...
        count = 0
        with open(filePath + "/3wikidataRelation/readytoCrawl.json", "r") as fr:
            for line in fr.readlines():
                count += 1
                if count % 10 == 0:
                    print(1.0 * count / 3986)
        ...
        for section in response.xpath('//h2[contains(@class,"wb-section-heading")]//span/text()'):
            ...
            if(title == "Statements"):
                ...
                for statement in response.xpath('.//div[@class="wikibase-statementgroupview"]'):
                    relationItem = statement.xpath(
                        './/div[@class="wikibase-statementlistview"]')
                    relationName = statement.xpath(
                        './/div[contains(@class,"wikibase-statementgroupview-property-label")]//a[contains(@title,"P")]/text()').extract()
                    ...
                    for relatedEntity in relationItem.xpath('.//div[contains(@class,"wikibase-statementview-mainsnak")]//div[contains(@class,"wikibase-statementview-mainsnak")]\
                        ...
                        if(len(entityId) == 0):
                            ...
                            httpRequest.proxies = {'http': 'http://127.0.0.1:1084',
                                                   'https': 'https://127.0.0.1:1084'}
                            httpRequest.mount(
                                'https://', HTTPAdapter(max_retries=30))
                            httpRequest.mount(
                                'http://', HTTPAdapter(max_retries=30))
                            url = "https://www.wikidata.org/w/api.php?action=wbgetentities&ids=" + \
                                relatedEntityId + "&format=json"
                            relatedEntityJson = httpRequest.get(
                                url, headers=headers).json()
                            httpRequest.close()
                            entity2 = str()
                            if 'zh' in relatedEntityJson['entities'][relatedEntityId]['labels']:
                                entity2 = relatedEntityJson['entities'][
                                    relatedEntityId]['labels']['zh']['value']
                            elif 'en' in relatedEntityJson['entities'][relatedEntityId]['labels']:
                                entity2 = relatedEntityJson['entities'][
                                    relatedEntityId]['labels']['en']['value']
                            else:
                                continue
