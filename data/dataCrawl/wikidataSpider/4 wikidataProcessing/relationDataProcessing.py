...
class loadDatatoNeo4j(object):
    ...
    def connectDB(self):
        self.graph = Graph("http://localhost:7474",
                           username="neo4j", password="111111", secure=False, bolt=False)
    ...
    def readData(self):
        ...
        with open("../3wikidataRelation/entityRelation.json", "r") as fr:
                ...
                with open("wikidata_relation.csv", 'a') as fwWikidataRelation:
                    with open("wikidata_relation2.csv", 'a') as fwWikidataRelation2:
                        ...
                        for line in fr:
                            ...
                            find_entity1_result = self.matcher.match(
                                "HudongItem", title=entity1).first()
                            # 搜索entity2

                            find_entity2_result = self.matcher.match(
                                "HudongItem", title=entity2).first()
                            count += 1
                            print(count / 264092)
